...
k = df.iloc[1, 0]
rptDate = (
    k.date() if k == k else prior_month_end().date()
...
start_time = time.time()
print(
    "Copying PSIF, PLMED, GACASH, GAEMBF, and ECICBALC reports \
to their respective reporting folders ..."
)
...
# to folders
# fldr_ECICBALC    = r'P:\Investment Operations\Segregated Clients\Active Clients\Export Credit Insurance Corporation\ECICBAL\Reporting\Monthly Reports'
...
# copy PSIF to new folder at PManco
frm = os.path.join(pthReports, fln_PSIF_Reg30)
to = os.path.join(fldr_PManco1, f"{rptDate.strftime('%Y%m%d')} PSIF Reg30.xlsx")
if os.path.exists(frm):
    shutil.copyfile(frm, to)

# copy PSIF to second folder at PManco
frm = os.path.join(pthReports, fln_PSIF_Reg30)
to = os.path.join(fldr_PManco2, f"{rptDate.strftime('%Y%m%d')} PSIF Reg30.xlsx")
if os.path.exists(frm):
    shutil.copyfile(frm, to)
...
filenames = [fln_ECIC_Reg28, fln_ECIC_Tbl2, fln_ECIC_SchIB]
...
print(
    f" {timediff(start_time, time.time())} copying PSIF, \
PLMED, GAEMBF, and ECICBALC reports to their \
respective reporting folders\n"
)


# get list of month-end 17 portfolio codes
res = pd.read_excel(pthPy, sheet_name="arc", usecols="AF").dropna()
me17_list = [s + "_EXP" for s in res.iloc[:, 0]]
s = "" if len(me17_list) == 1 else "s"
print(f"{len(me17_list)} fund{s} to export: \n   {(', ').join(me17_list)}")
...
print(
    f" {timediff(start_time, time.time())} creating and then saving individual look-through holdings report workbooks"
)
