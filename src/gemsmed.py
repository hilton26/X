...
libraries to get fund holdings\n",
)
...
df = pd.read_excel(pthPy, sheet_name="arc", usecols="K").dropna()  # ptfl codes
df1 = pd.read_excel(pthPy, sheet_name="arc", usecols="L", nrows=2)  # rpt dates
k = df1.iloc[1, 0]
rptDate = (
    k
...
print(
    f" {rptDate.strftime('%a %d %b %Y')} for \
{len(df.iloc[:, 0])} fund{s}:\n  {funds}"
)
...
start_time = time.time()
print("Extracting and saving the fund \
...
print(
    f"\n {timediff(start_time, time.time())} extracting \
and saving the fund holdings and derivatives\n",
)

# prepare the combined holdings and derivatives dataframe

start_time = time.time()
print(f"Preparing the combined holdings and \
derivatives dataframes ...\n")
...
print(" Converting numerical columns from \
type string to type float ...")
...
print(
    f"  {timediff(start_time, time.time())} converting \
numerical columns from type string to type float"
)
print(
    f"\n{timediff(start_time, time.time())} preparing \
the combined holdings and derivatives dataframes\n",
)
...
a["Closing Exposure PA"] = np.where(
    a["Effective Exposure"].isnull(), 
    a["End Market Value"], 
    a["Effective Exposure"]
)

# make SYTH values negative
a["Closing Exposure PA"] = np.where(
    a["Investment Type"] == "SYTH", 
    -a["Closing Exposure PA"], 
    a["Closing Exposure PA"]
)
...
print(
    "", f"{a['End Market Value'].sum() - a['Closing Exposure PA'].sum()} \
NAV difference"
)
...
print(f" {timediff(start_time, time.time())} running r_classifier")
print(
    os.path.join(
        Path.home(),
        "Documents",
...
print(
    f"\n{timediff(start_time_gems_pre, time.time())} roundtrip \
time to get fund holdings\n",
)
