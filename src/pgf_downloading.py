...
# derive names of the files to be donloaded
utP_fln = f"UTPS PGF_UT_Prices({len(df['pgf: UT prices'].dropna())}) {rptDate.strftime('%d%b%Y')}.csv"
parN_fln = f"PARN PGF_Holdings({len(df['pgf: PAR-N'].dropna())}) {rptDate.strftime('%d%b%Y')}.csv"

# derive paths of the files to be downloaded
parN_nm = os.path.join(pth_dl, parN_fln)
utP_nm = os.path.join(pth_dl, utP_fln)

# print(os.path.isfile(parN_nm))
# print(os.path.isfile(utP_nm))
...
try:
    ...
    if os.path.isfile(utP_nm):
        ...
        print(
            f"  {timediff(start_time_prices, time.time())} \
getting PGF unit trust prices"
        )
...
try:
    ...
    if os.path.isfile(parN_nm):
        ...
        print(
            f"  {timediff(start_time_navs, time.time())} getting \
PGF unit trust prices"
        )
except Exception as e:
    print(e)
...
class holdings report\n",
)
print(
    f" {timediff(start_time_pgf_dl, time.time())} roundtrip \
time to get unit trust prices and holdings reports\n",
)

print(f"{rptDate.strftime('%a %d %B %Y')} downloads")
print(
    f"  {parN_nm} which \
{'exists' if os.path.exists(parN_nm) else 'does not exist'}"
)
print(
    f"  {utP_nm} which \
{'exists' if os.path.exists(utP_nm) else 'does not exist'}"
)
